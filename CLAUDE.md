...
- **Tool page shell**: there is no shared shell component today — `ToolWorkspaceShell.tsx` existed briefly but was deliberately deleted (commit `35d80a1`, "Reposition site as WordPress + Web Systems + Web Tools...") in favor of giving each of the 15 tool pages its own bespoke, workflow-specific layout. Treat the current per-page layouts as the intended architecture, not tech debt — this note previously (incorrectly) called that inconsistency "unfinished work." A new shared *presentational* shell (`ToolPageChrome`, planned for the rebuild's Phase 5 — see the design plan referenced in project memory) is a different, later decision: it unifies page chrome (header/breadcrumb/guide/related-tools/CTA) only, not the per-tool workspace logic components, which stay separate.
